...
//WRONG          self.displayResultLable.text +=  (title as! String)
//CORRECT        self.displayResultLable.text =  self.displayResultLable.text! + title as! String
        
        if (functionArguments?.count > 0) {
            functionName = functionName! + "(\(dataToJSONString(functionArguments!)!))"
        } else if let aString = functionArguments as? String {
            functionName = functionName! + "(\"" + aString + "\")"
    ...
    fileprivate func callJSFunction(_ jsHandler: Dictionary<String, AnyObject>) {
        ...
        self.jsWebView?.evaluateJavaScript(functionName!, completionHandler: { (response : Any?, error: Error?) -> Void in
            if (error != nil) {
                print(kEvaluateScriptErrorText + functionName! + " - " + String(describing: error))
            } else {
                if (response != nil && callBackClosure != nil) {
                    callBackClosure!(response! as AnyObject?)
                }
            }
        })
    }
